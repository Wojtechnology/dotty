...
import dotty.tools.dotc.reporting.diagnostic.messages.{UnapplyInvalidResultType, UnapplyInvalidNumberOfArguments}
...
object Applications {
  ...
  def unapplyArgs(unapplyResult: Type, unapplyFn: Tree, args: List[untpd.Tree], pos: Position = NoPosition)(implicit ctx: Context): List[Type] = {
    ...
    def fail = {
      ctx.error(UnapplyInvalidResultType(unapplyResult, unapplyFn), pos)
      Nil
    }
